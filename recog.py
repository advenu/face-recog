...
def save_cropped_images(img_path, predictions, output_dir):
    """
    Saves the cropped images of detected faces.
    ...
    for name, (top, right, bottom, left) in predictions:
        # Crop the image using detected coordinates
        cropped_image = pil_image.crop((left, top, right, bottom))

        # Calculate the hash of the cropped image
        cropped_image_hash = hashlib.sha256(cropped_image.tobytes()).hexdigest()[:16]

        user_dir = os.path.join(output_dir, name)
        Path(user_dir).mkdir(exist_ok=True)

        # Construct the filename using the name and hash of the cropped image
        filename = os.path.join(user_dir, f"{cropped_image_hash}{ext}")
